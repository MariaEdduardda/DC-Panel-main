...
def adicionar_ocorrencia_na_tabela(nome_arquivo, tempo_video, data_ocorrencia_arquivo):
    """
    Adiciona uma nova ocorrência ao banco de dados e atualiza a tabela,
    ...
    try:
        ...
        # 🔹 Recarrega as ocorrências do banco e atualiza a tela
        carregar_ocorrencias_do_banco()
        mudar_pagina("Ocorrências", "Conteúdo da página de ocorrências.", "ocorrencias")

        messagebox.showinfo("Sucesso", "Ocorrência registrada e salva no banco de dados!")

    except Exception as e:
        messagebox.showerror("Erro", f"Ocorreu um erro ao salvar a ocorrência: {e}")
...
def upload_arquivo_ocorrencia():
    ...
    try:
        filepath = filedialog.askopenfilename(
            ...
            filetypes=[("Arquivos de Vídeo", "*.mp4;*.avi;*.mov;*.jpg"),
                       ("Todos os arquivos", "*.*")]
        )
        if filepath:
...
def carregar_ocorrencias_do_banco():
    """Carrega todas as ocorrências salvas no banco de dados."""
